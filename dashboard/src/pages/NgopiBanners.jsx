...
export default function NgopiBanners() {
    ...
    const fetchBanners = async () => {
        ...
        try {
            ...
            if (data.ngopi_banners) {
                let fetchedBanners = [];
                if (Array.isArray(data.ngopi_banners)) {
                    fetchedBanners = data.ngopi_banners;
                } else if (data.ngopi_banners.banners && Array.isArray(data.ngopi_banners.banners)) {
                    fetchedBanners = data.ngopi_banners.banners;
                }
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await fetch('/api/settings', {
                ...
                body: JSON.stringify({
                    ngopi_banners: validBanners
                })
            });

            if (res.ok) {
                alert('Banner Ngopi berhasil disimpan!');
                fetchBanners();
            } else {
        ...
        <div className="min-h-screen bg-bg flex">
            ...
            <main className="flex-1 ml-64 p-8">
                <header className="flex justify-between items-center mb-8">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-800">Banner Ngopi</h1>
                        <p className="text-gray-500">Kelola banner horizontal di halaman Ngopi</p>
                    </div>
                    <button
                        onClick={handleAddBanner}
                        className="flex items-center gap-2 bg-amber-600 text-white px-4 py-2 rounded-xl hover:bg-amber-700 shadow-lg shadow-amber-600/20 font-bold"
                    >
                        <Plus size={18} />
                        Tambah Banner
                    </button>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                            ...
                            <p className="text-xs text-gray-500 mt-2">
                                Rekomendasi ukuran: 192x192px (rasio 1:1)
                            </p>
                        </div>
                            ...
                            {banners.length === 0 ? (
                                ...
                                <div className="space-y-4">
                                    {banners.map((banner, index) => (
                                        <div
                                            key={banner.id}
                                            className="border border-gray-200 rounded-xl p-4 hover:border-amber-300 transition"
                                        >
    ...
    return (
        <div className="w-32 h-24 rounded-2xl overflow-hidden flex-shrink-0 relative bg-white shadow-sm border border-gray-200">
            ...
            {banner.link && !imageError && (
                <div className="absolute top-2 right-2 bg-amber-500 text-white p-1 rounded-full" title={`Link: ${banner.link}`}>
