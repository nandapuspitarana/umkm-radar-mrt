...
export default function SarapanBanners() {
    ...
    const fetchBanners = async () => {
        ...
        try {
            ...
            if (data.sarapan_banners) {
                let fetchedBanners = [];
                if (Array.isArray(data.sarapan_banners)) {
                    fetchedBanners = data.sarapan_banners;
                } else if (data.sarapan_banners.banners && Array.isArray(data.sarapan_banners.banners)) {
                    fetchedBanners = data.sarapan_banners.banners;
                }
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await fetch('/api/settings', {
                ...
                body: JSON.stringify({
                    sarapan_banners: validBanners
                })
            });

            if (res.ok) {
                alert('Banner Sarapan berhasil disimpan!');
        ...
        <div className="min-h-screen bg-bg flex">
            ...
            <main className="flex-1 ml-60 p-8">
                <header className="flex justify-between items-center mb-8">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-800">Banner Sarapan</h1>
                        <p className="text-gray-500">Kelola banner horizontal di halaman Sarapan</p>
                    </div>
                    <button
